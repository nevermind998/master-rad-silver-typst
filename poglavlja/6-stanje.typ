...
def generate_k8s_manifests(model: SilveraModel, output_dir: Path) -> list[Path]:
	...
	, caption: [Функција за генерисање Kubernetes манифеста],
 	...
 	Функција generate_k8s_manifests се састоји од три фазе. Прва фаза је припрема, где се инстанцира Jinja2 Environment, друга фаза итерира кроз model.services и за сваки ServiceDef рендерује deployment.j2. Из модела користи само три вредности, а то су име, порт и изведени назив конекционог стринга, резултат ове фазе су четири манифест датотеке. На крају се праве заједнички манифести (yaml фајлови) namespace, configMap, secret и ingress. Ови не иду кроз шаблон него су hardcoded Python стрингови, резултат ове фазе су 8 фајлова. Једини фајл који је потребно ручно написати је rabbitmq.yaml. 
