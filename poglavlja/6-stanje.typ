...
== Генерисање Docker конфигурације
 	...
 	Генерисање Docker фајлова у Silverа-и функционише на тај начин да имамо четири Dockerfile-a и један compose фајл. Dockerfile-ови иду кроз Jinja2  шаблон, док је compose укуцан као Python string. На слици @slika-6-3-3-1 је приказан како се по сервису од модела дође до Docker фајла 
...
#figure(image("../slike/slika-6.3.3-1.png", width: 90%), caption: [Дијаграм који показује пут од модела до Docker фајла по сервису]) <slika-6-3-3-1>
	...
	Docker фајл у Silverа-и се генеришу по сервисима. Основни шаблон је приказан на слици @slika-6-3-3-2, мењају се само service.name и service.port у зависности за који сервис се генеришу. 
...
Укупан трошак ових сервиса за последњих три месеца је приказан на слици @slika-6-10-3-3.
 
#figure(image("../slike/slika-6.10.3-3.png", width: 90%), caption: [Преглед трошкова за три месеца]) <slika-6-10-3-3>
Сви сервиси су смештени у оквиру Kubernetes сервиса order-tracking-aks, јер је то био једноставнији начин како би се покретање свих сервиса уклопило у претплату за Azurе налог који користим. Преглед тога видљив је на слици @slika-6-10-3-4.
...
Главна мотивација је да се смањује количина ручног рада и да се упрости процес постављања микросервисне апликације. Из тих разлога јавила се потреба да се што већи део животног циклуса развоја софтвера подржи у оквиру Silvera-е. Проширењем Silvera-е тако да се генеришу неопходни артефакат за пребацивање на Azure платформу, би у великој мери била смањена количина ручног рада и поједностављује процес пребацивања микросервисне апликације. Омогућава аутоматизовано тестирање успешно пребачених сервиса, чиме се обезбеђује да генерисана апликација не само да буде исправно креирана, већ и функционално проверена након постављања.
...
Како би пребацивање на Azure било могућ потребно је да постоји претходно креиран налог, који има одређене претплате које подржавају покретање ових сервиса. За потребе овог мастер рада, након направљеног налога, активирала сам претплату Azure за студенте   и добила 100 долара бесплатног кредита који могу да користим. Када постоји креиран налог са активном претплатом потребно је да се одради команда az acr login, након ње docker compose build. На слици @slika-6-10-3-1 се види како то све изгледа након успешног пребацивања апликације на Azure-у. Све ово је смештено у оквиру order-tracking-rg ресурс групе.
