...
== Генерисање Docker конфигурације
 	...
 	Генерисање Docker фајлова у Silvera-и функционише на тај начин да имамо четири Dockerfile-а и један compose датотека. Dockerfile-ови иду кроз Jinja2 шаблон, док је compose укуцан као Python string. На слици @slika-6-3-3-1 је приказано како се по сервису од модела дође до Docker фајла.

 

#figure(image("../slike/slika-6.3.3-1.jpg", width: 90%), caption: [Дијаграм који показује пут од модела до Docker фајла по сервису]) <slika-6-3-3-1>
...
У оквиру Silvera-е главни део модела за Terraform је написан у датотекама `aws-deployment.si` и `azure-deployment.si`. Унутар ових фајлова је дефинисано како да се направи све што је потребно за AWS/Azure налог, затим регистровање контејнера и Kubernetes кластера, дефинисање базе, начина слања порука, мониторинг и потребан фајл у коме се чувају скривене битне информације везане за базу и сервисе. Након генерисања апликације добијају се два одвојена фолдера за AWS и Azure, који имају исту структуру. Структура је приказана у листингу @listing-6-9-1.
...
def generate_k8s_manifests(model: SilveraModel, output_dir: Path) -> list[Path]:
	...
	, caption: [Функција за генерисање Kubernetes манифеста],
 	...
 	Функција generate_k8s_manifests се састоји од три фазе. Прва фаза је припрема, где се инстанцира Jinja2 Environment, друга фаза итерира кроз model.services и за сваки ServiceDef рендерује deployment.j2. Из модела користи само три вредности, а то су име, порт и изведени назив конекционог стринга, резултат ове фазе су четири манифест датотеке. На крају се праве заједнички манифести (yaml фајлови) за namespace, configMap, secret и ingress. Ови не иду кроз шаблон него су унапред задати Python стрингови, резултат ове фазе су 8 фајлова. Једини фајл који је потребно ручно написати је rabbitmq.yaml. 

У случају саме апликације за обраду поруџбине генерисане уз помоћ Silvera-е прво Terraform направи празан кластер. Kubernetes манифест фајлови га онда попуњавају изгенерисаним сервисима. Важно је напоменути да се Terraform покреће једном, док се манифест фајлови покрећу приликом сваког deploy-а. Укратко, Docker све спакује, Terraform прави место где ће радити, а  Kubernetes покреће. За микросервисну архитектуру, где постоји велики број независних сервиса, Terraform и Kubernetes доста поједностављују управљање системом. Terraform омогућава поновљиво креирање истих окружења (development, testing, production), док Kubernetes обезбеђује поуздано извршавање сервиса и динамичко прилагођавање оптерећењу. Управо из ових разлога Terraform и Kubernetes су циљне технологије које се користе за аутоматизацију самог процеса пребацивања апликације на облачне платформе, који се генеришу коришћењем Silvera-е. ЈСД може поједноставити сложеност ових технологија тако што корисник описује архитектуру система на вишем нивоу, док се конкретне Terraform и Kubernetes конфигурације генеришу аутоматски.
 
...
Веома је важно да постоји одређен начин провере рада сервиса након што се апликација и њени сервиси пребаце на Azure. Апликација за обраду поруџбине, која је описана у овом мастер раду, део тестова је генерисан уз помоћу језика Silvera, док је један део писан ручно. Ручно писани су такозвани smoke тестови. Не постоји разлика између генерисаних тестова за Azure и AWS,  ради се на исти начин. 

Сам приступ за генерисање тестова функционише другачије од већ описаних процеса за фајлове међу којима су services.si, events.si, communication.si, entities.si. У оквиру ових фајлова постоји додатак који генератор test_generator.py користи и уз помоћ Jinja шаблона штампа се као стварни Python код, помоћу кога се ствара прави Python фајл. Тестови који су изгенерисани и који су смештени у tests/generated/ су api_surface.py, ту имамо чисте податке, међу којима је листа свих рута и топологија догађаја. Изгенерисан тест test_auth_enforcement.py тестира јавне руте, тестира руте без токена и са неважећим токеном, постоје и тест примери који покривају када се приступа са исправним токеном. Тест test_jwt_hardening.py испитује токене по сервису, тест test_messaging.py је задужен за испитивање порука. Што се самих догађаја тиче, покривени су тестовима тако што се проверава да ли постоји RabbitMQ exchange за сваки догађај који неко користи, проверава се да ли за сваки модел за који треба да постоје редови везани за exchange заправо постоје. Битно је да се види да ли сваки ред има бар једног активног consumer-а и да ли је durable (ако модел то тражи), да ли dead-letter ред постоји тамо где је декларисан и бави се пријављивањем „orphan" догађаја на информативном нивоу. Ручно писани тестови су smoke тестови и то је урађено из разлога што су то тестови којима се тестира имплементација, а не сам ЈСД модел.
...
Када имамо све проверено на овај начин, осигурано је да се покренути тестови извршавају на Azure платформи. Покретање тестова се врши тако што се прво навигира до директоријума где се налазе тестови, а затим покрену следећим командама
...
=== Тестирање имплементираног решења на AWS платформи
 	...
 	Као што је већ споменуто генерисање тестова за Azure и AWS се одвија на исти начин, јер су фајлови services.si, events.si, communication.si, entities.si где се налазе Silvera модели проширени како би подржали генерисање тестова. Овде су такође smoke тестови ручно направљени из истих разлога, јер се њима тестира сама имплементација, а не сами ЈСД модели. Детаљније о самим тестовима и њиховом дефинисању у оквиру апликације налази се у поглављу које се бави тестирањем имлементираног решења на Azure платформи. Пошто су исти тестови, како би тестирали њихово понашање на различитим платформама, битан је начин на који се све ово покреће. Међу првим корацима је то да се преко терминала улогује на одговарајући AWS налог, где су постављени сервиси. Потребно је покренути команде, чији излаз би требало да буде running 3.84.174.154
...
Проверити да ли се као статус враћа вредност health, за следеће команде
