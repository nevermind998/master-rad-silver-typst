...
Главне предности су у томе што је синтакса јако једноставна, брзо се генерише CRUD апликација, добра је интеграција са JHipster платформом. Поред ових предности, постоје одређени недостаци који се односе на то да је ограничено на JHipster платформу, није подржано инфраструктурно моделовање, није погодна за сложеније микросервиснe архитектура. Постоји документација која детаљније описује на који начин је могуће да се инсталира и користи#footnote[https://www.jhipster.tech/getting-started]. 

== Structurizr DSL

Structurizr DSL#footnote[https://docs.structurizr.com/dsl] је језик специфичан за домен уз помоћ кога се добија текстуално моделоване софтверске архитектуре на основу C4  модела. C4#footnote[https://c4model.com/] модел (Context, Containers, Components, Code) представља приступ за визуелно моделовање софтверске архитектуре. Овај модел приказује архитектуру система кроз четири нивоа апстракције, што олакшава разумевање система људима који су из различитих професија (архитекте, програмери, аналитичари и менаџери). Structurizr DSL подржава моделовање свих нивоа C4 модела: System Landscape, System Context, Container, Component, Dynamic, Deployment дијаграма. @structurizr-dsl
...
Structurizr DSL даје опис система, контејнера, компоненти и њихових међусобних односа. Користи приступ који се назива Architecture as Code или Models as Code, где се архитектура чува у облику изворног кода, може се верзионисати у Git репозиторијумима и развијати заједно са самом апликацијом. Другим речима, то значи да се архитектура система описује као текст, после се из тога генерише модел који је архитектурни дијаграми. Поред основног моделовања архитектуре, језик подржава и бројне напредне могућности, као што су дефинисање архетипова (Archetypes) ради поновне употребе модела, аутоматско закључивање имплицитних односа између елемената, укључивање других ЈСД датотека (Includes), проширење постојећих workspace модела, интеграцију Markdown и AsciiDoc документације, вођење евиденције архитектурних одлука (Architecture Decision Records - ADRs), коришћење скрипти и додатака (Plugins) за аутоматизацију, извоз модела у различите формате, укључујући PlantUML  и друге алате за визуелизацију. @structurizr-dsl @predoaia2025
...
Предност Structurizr DSL-а је да постоји јединствен модел, који генерише више архитектурних приказа различитог нивоа апстракције. Сви дијаграми су међусобно конзистентни, разлог за тим је што када год се измени текстуални модел, одмах након тога приказ се ажурира. Недостаци су то што не може аутоматски да се генерише имплементацијa, не постоји описа API-ја, нити се моделира инфраструктура. Он не моделира детаљну пословну логику, не генерише изворни код апликације нити конфигурације за распоређивање на облак платформи. Structurizr DSL се најчешће користи као средство за документовање архитектуре и комуникацију између архитеката, програмера и других заинтересованих страна у процесу развоја софтвера. 

 

== Smithy

Smithy#footnote[http://smithy.io/2.0/]  је језик за описивање сервиса (Interface Definition Language - IDL) који је развио AWS (Amazon Web Services). Основна намена овог језика је да омогући моделовање API-ја и аутоматско генерисање програмског кода, документације и SDK-ова за различите програмске језике. Настао је тако што га Amazon#footnote[https://www.amazon.com/] интерно користи више од десет година за развој AWS сервиса, као и за развој десетина хиљада других сервиса. У једном тренутку компанија Amazon је од Smithy направила open-source#footnote[https://opensource.com/resources/what-open-source]   пројекат како би и други програмери могли да користе исти приступ приликом развоја својих сервиса. Smithy није ограничен само на коришћење у оквиру AWS-а, може користити за развој сервиса у облаку, микросервисних система, интерних корпоративних API-ја, јавних REST API-ја, RPC система. Основне карактеристике су моделовање сервиса, дефинисање операција, моделовање типова података, опис REST и RPC сервиса, генерисање клијентских библиотека. Модели који се користе су потпуно нормализовани, сваки тип података има јединствено име и јасно дефинисано место у моделу. Такође је омогућено да различити тимови управљају различитим деловима модела. Савремени Smithy омогућава моделовање REST, RPC и сервиса у облаку, генерисање SDK-ова за више програмских језика (Java, Kotlin, TypeScript, Rust, Python, C\# и друге), генерисање серверског кода и клијентских библиотека, генерисање OpenAPI спецификација, валидацију модела и проверу усклађености са правилима, проширивање модела путем traits-а, генерисање техничке документације, интеграцију са AWS сервисима и алатима, али и употребу у потпуно независним системима. @smithy
...
OpenAPI  се може користи и за генерисање Azure Functions. Примери су .NET Azure Functions, Java Azure Functions. Омогућава им да користе OpenAPI JSON екстензију која генерише #footnote[https://swagger.io/open-source/swagger-ui/]  и на тај начин је омогућено тестирање функција преко прегледача. 
...
Као што је већ наглашено и истакнуто, овим приступом добијамо јасну разлику између архитектуре и сервисног модела, што само по себи има неке предности и недостатке. Предност је то што се измене у архитектури могу вршити потпуно независно од API спецификације, а уколико постоје неке измене на API-јима то се не одражава на већ постојеће архитектурне дијаграме, генерисане коришћењем Structurizr DSL. Апликација направљена овом комбинацијом, има јасну поделу одговорности, што повећава модуларност система и даје могућност да сваки језик буде употребљен у области за коју је оптимизован. Недостаци се огледају у томе како архитектура и API нису описани у оквиру једног модела, потребно је да се одржава њихова међусобну синхронизацију. Ако се дода нови микросервис у архитектурном моделу, након тога се ручно креира одговарајући Smithy модел. Доменски модели, као што су ентитети, релација и пословних правила, као и конфигурацију за пребацивање на облачне платформе мора ручно да се креира. 
 


=== Поређење Silvera-е са комбинацијом језика Smithy и Structurizr DSL
...
=== Поређење Silvera-е са комбинацијом језика ЈHipster Domain Language и OpenAPI
...
Предности су те што је систем модуларнији, јер имамо јасно разграничење одговорности. Недостаци овог приступа су то што постоје два независна модела, којима се описују различити аспекти система. Доменски модел је дефинисан уз помоћ JDL-а и API модел дефинисаног у Smithy-у морају се одржавати синхронизовано. Све измене које се десе захтевају ажурирање оба модела, што повећава сложеност развоја и могућност појаве неконзистентности. Не постоји модел комплетне архитектуре система, већ су покривени само посебни сегменти, како би се постигло то да имамо документацију у целости, морају да се користе Structurizr DSL или UML дијаграми. Како би апликација била подржана на облачним платформама, као што су Azure или AWS за то неопходно користити додатне алате и конфигурационе датотеке.

 


=== Поређење Silvera-е са комбинацијом језика ЈHipster Domain Language и Smithy
...
Када се узму у обзир сами захтеви система, технологије и остало може се одлучити шта је прави избор. У табели испод, за различите критеријуме имаћемо бољу слику за ово поређење. 
...
Прво је потребно да се Smithy креира модел система, затим да се OpenAPI спецификација аутоматски генерише из Smithy модела. OpenAPI спецификација се користи за документацију, интеграцију са другим системима и алатима, као и за генерисање клијентских библиотека које користе програмери. На горе описан начин се избегава то да морају да се одржавају два независна API модела и обезбеђује конзистентност између имплементације сервиса и документације. Уколико је потребно да се измени API модел, то се прво ради у оквиру Smithy спецификације, после тога се OpenAPI документација може аутоматски генерисати. То значајно смањује могућност појаве неконзистентности између имплементације сервиса и његове документације.
...
=== Поређење Silvera-е са комбинацијом језика OpenAPI и Smithy
